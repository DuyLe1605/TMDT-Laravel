    ...
    <div class="col-xl-11 col-xxl-10">
        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" id="productForm">
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
                <div>
                    <h3 class="fw-bold text-dark mb-1" style="letter-spacing: -0.02em;">Chỉnh Sửa Túi Xách: {{ $product->name }}</h3>
                    <p class="text-secondary small mb-0">
                        Cập nhật thông tin chi tiết, thương hiệu, danh mục và cấu hình bảng giá các biến thể
                    </p>
                </div>
                <div class="d-flex align-items-center gap-2">
                    ...
                    <a href="{{ route('shop.show', $product) }}" target="_blank" class="btn-surface text-primary" title="Xem trên sàn TMĐT">
                        <i data-lucide="external-link" style="width: 15px; height: 15px; margin-right: 0.4rem;"></i>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8">
                    ...
                    <div class="card-modern p-4 mb-4">
                        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 pb-3 border-bottom mb-3">
                            <div>
                                ...
                                <div class="text-secondary small">
                                    Cấu hình các phân loại (như <em>Chất liệu</em>, <em>Màu sắc</em>, <em>Kích thước</em>) và bảng giá chi tiết
                                </div>
                            </div>
                        ...
                        <div id="simplePricingSection" class="{{ old('has_variants', $product->has_variants) ? 'd-none' : '' }}">
                            <div class="row g-3">
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input 
                                        ...
                                        id="sale_price" 
                                        name="sale_price" 
                                        value="{{ old('sale_price', $product->sale_price !== null ? (float)$product->sale_price : '') }}" 
                                    >
                                </div>
                                <div class="col-md-4">
                        ...
                        <div id="variantsEngineSection" class="{{ old('has_variants', $product->has_variants) ? '' : 'd-none' }}">
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card-modern p-4 mb-4">
                        ...
                        <div class="rounded-3 border overflow-hidden d-flex align-items-center justify-content-center mb-3" style="height: 190px; background: var(--bg-surface-subtle);" id="mainImagePreviewBox">
                            @if ($product->image)
                                <img src="{{ $product->image }}" alt="Preview" class="w-100 h-100 object-fit-cover">
                            @else
                    ...
                    <div class="card-modern p-4 mb-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control form-control-modern" id="material" name="material" value="{{ old('material', $product->material) }}">
                        </div>

                        <div class="mb-3">
                            <label for="dimensions" class="form-label-modern mb-1">Kích thước (D x R x C)</label>
                            <input type="text" class="form-control form-control-modern" id="dimensions" name="dimensions" value="{{ old('dimensions', $product->dimensions) }}">
                        </div>

                        <div class="mb-3">
                            <label for="color" class="form-label-modern mb-1">Màu sắc chủ đạo</label>
                            <input type="text" class="form-control form-control-modern" id="color" name="color" value="{{ old('color', $product->color) }}">
                        </div>
                    </div>
...
@php
    ...
    if (empty($initialGroups) && $product->has_variants) {
        $initialGroups = [
            ['name' => 'Chất liệu', 'values' => ['Da thật', 'Da PU']],
            ['name' => 'Màu sắc', 'values' => ['Đen', 'Nâu']]
        ];
    }

    $initialVariantsCache = [];
    foreach ($product->variants as $var) {
        $initialVariantsCache[$var->variant_title] = [
            'sku' => $var->sku,
            'price' => (float)$var->price,
            'sale_price' => $var->sale_price !== null ? (float)$var->sale_price : '',
            'stock' => (int)$var->stock,
            'image' => $var->image,
...
<script>
    ...
    let attributeGroups = @json($initialGroups);
    let variantValuesCache = @json($initialVariantsCache);
            ...
            groupCard.innerHTML = `
                <div class="d-flex align-items-center justify-content-between gap-3 mb-2">
                    <div class="d-flex align-items-center gap-2 flex-grow-1">
                        ...
                        >
                        <span class="text-secondary small d-none d-md-inline">Gợi ý: 
    ...
    function generateMatrix() {
        ...
        let baseSku = document.getElementById('sku')?.value || 'BAG';
        ...
        combinations.forEach((combo, idx) => {
            ...
            const sku = cached.sku || (baseSku + '-' + combo.map(v => v.substring(0, 3).toUpperCase().replace(/[^A-Z0-9]/g, '')).join('-'));
            const price = cached.price !== undefined && cached.price !== '' ? cached.price : defaultBasePrice;
            const salePrice = cached.sale_price !== undefined ? cached.sale_price : defaultBaseSalePrice;
            const stock = cached.stock !== undefined && cached.stock !== '' ? cached.stock : defaultBaseStock;
            const image = cached.image !== undefined ? cached.image : mainImage;
